...
import { ArrowUp, Check, ChevronLeft, Clock, Mic, Sparkles } from "lucide-react";
...
function ResultCard({ result, onAddAll }: { result: SpeakResult; onAddAll: () => void }) {
  return (
    ...
    >
      {result.recipe && (
        <div className="relative h-28">
          ...
          <div className="absolute bottom-2 left-3 right-3 text-white">
            <p className="font-bold text-[15px]">{result.recipe.name}</p>
      ...
      {result.dietary_note && (
        <div className="bg-amzn-greenlite text-amzn-green text-[12px] font-semibold px-3 py-2 flex items-center gap-1.5">
          <Check size={14} /> {result.dietary_note}
        </div>
      )}
      <div className="px-3 divide-y divide-line/70">
      ...
      {result.products.length > 0 && (
        <button
          onClick={onAddAll}
          className="m-3 w-[calc(100%-1.5rem)] rounded-xl bg-amzn-yellow2 text-amzn-dark font-bold py-3 flex items-center justify-center gap-2"
        >
          Add all {result.products.length} · {rupee(result.total)}
        </button>
      )}
    </motion.div>
  );
